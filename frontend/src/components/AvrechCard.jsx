...
export default function AvrechCard({ avrech, onChanged, defaultOpen = false }) {
  const [open, setOpen] = useState(defaultOpen);
  const [newText, setNewText] = useState("");
  const [newMonthlyReminder, setNewMonthlyReminder] = useState(false);
  const [adding, setAdding] = useState(false);
  const [editingId, setEditingId] = useState(null);
  const [editingText, setEditingText] = useState("");
  const [editingMonthlyReminder, setEditingMonthlyReminder] = useState(false);
  ...
  async function handleAdd(e) {
    ...
    setAdding(true);
    try {
      await createUpdate(avrech.id, text, newMonthlyReminder);
      setNewText("");
      setNewMonthlyReminder(false);
      await onChanged();
    } finally {
      setAdding(false);
    }
  ...
  function startEdit(update) {
    ...
    setEditingMonthlyReminder(update.monthly_reminder || false);
  }
  ...
  async function submitEdit(e) {
    ...
    if (!text) return;
    await editUpdate(editingId, text, editingMonthlyReminder);
    setEditingId(null);
    await onChanged();
  }
      ...
      {open && (
        <div className="avrech-card-body">
          <form className="update-form" onSubmit={handleAdd}>
            ...
            />
            <label className="monthly-reminder-checkbox">
              <input
                type="checkbox"
                checked={newMonthlyReminder}
                onChange={(e) => setNewMonthlyReminder(e.target.checked)}
              />
              תזכורת חודשית
            </label>
                ...
                {editingId === update.id ? (
                  <form className="update-edit-form" onSubmit={submitEdit}>
                    ...
                    <label className="monthly-reminder-checkbox">
                      <input
                        type="checkbox"
                        checked={editingMonthlyReminder}
                        onChange={(e) => setEditingMonthlyReminder(e.target.checked)}
                      />
                      תזכורת חודשית
                    </label>
                  ...
                  <>
                    <p className="update-text">
                      {update.monthly_reminder && (
                        <span className="monthly-reminder-badge">🔔 תזכורת חודשית</span>
                      )}
                      {update.text}
                    </p>
